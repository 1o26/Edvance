...
router.post('/assistant', async (req, res) => {
  try {
    ...
    let model;
    try {
      const modelWrapper = getModel(false);
...
Grade: ${plan.grade}
Content: ${JSON.stringify(plan.content, null, 2).substring(0, 500)}`;
        }
      } catch (error) {
    ...
    const prompt = `You are Edvance AI Assistant, a helpful educational AI that assists teachers with lesson planning, pedagogy, and educational questions.
...
Provide a helpful, concise, and educational response. If the question is about lesson planning, provide practical advice. If it's about pedagogy, reference educational best practices. Be friendly and supportive.

Response:`;

    try {
      const result = await generateWithRetry(model, prompt, `AI Chat Response`);
      ...
      if (is503Error) {
        return res.status(503).json({
          ...
          message: 'The AI assistant service is experiencing high load. Please try again in a few moments.',
          retryable: true
        });
      }
