...
class LessonPlanDataGenerator:
    ...
    def calculate_health_score(self, features_dict):
        ...
        # Engagement level - based on activity types (0-0.5)
        engagement_types = {'Group Discussion', 'Hands-on Experiment', 'Role Play', 'Project Work'}
        if any(activity_type in str(features_dict['activities']) for activity_type in engagement_types):
            score += 0.5
        
        # Content coverage - based on duration ratio (0-0.5)
        duration = features_dict['duration']
        content_complexity = features_dict['content_words'] / max(1, duration)
        if 10 <= content_complexity <= 30:
            score += 0.5
        elif content_complexity < 10 or content_complexity > 30:
            score += 0.2
